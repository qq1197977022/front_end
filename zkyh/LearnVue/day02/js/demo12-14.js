/*//使用全局API Vue.directive注册/ 获取全局自定义指令
Vue.directive('focus', {
    inserted(el) {  //钩子函数 ~ 被绑定元素插入父节点时调用
        /!*
        * el: 自定义指令所绑定的原始DOM元素对象*!/
        el.focus();
    }
});*/
...
new Vue({
    ...
    filters: {
        cimeFormat(ctimeObj) {
            ...
            mon = mon.toString().padStart(2, '0');    //完善ctime格式化 ~ 填充0
    ...
    directives: {   //通过组件选项directives定义私有自定义指令
        focus: {
            inserted(el) {
                el.focus();
            }
